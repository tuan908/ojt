...
public class CommentServiceImpl implements CommentService {
  ...
  public CommentServiceImpl(EntityManager entityManager,
      StudentEventRepository eventDetailRepository, UserRepository userRepository,
  ...
  public ApiResponse<Object> delete(Long studentEventId, Long commentId) {
    ...
    if (result == null) {
      return ApiResponse.error(ErrorCodes.NOT_FOUND.getValue(), messagesBundle.getString("error.not-found"),
          ResponseType.ERROR.getValue());
    }
    result.softDelete();
    this.entityManager.merge(result);
    this.entityManager.flush();
    return ApiResponse.success(null, messagesBundle.getString("ok"));
  }
}
