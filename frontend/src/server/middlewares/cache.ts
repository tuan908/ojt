...
import {generateRequestId, generateTimestamp} from '../lib/api-response';
import {
  defaultOptions,
  RedisCacheStore,
  type ICacheEntry,
...
export const createCacheMiddleware = (
  ...
  return async (c: Context, next: Next) => {
    ...
    if (cachedEntry) {
      ...
      const decodedBody = JSON.parse(cachedEntry.body) as IApiResponse; // No need to decode if we store properly
