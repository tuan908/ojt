...
public class ControllerExceptionHandler {
	...
	private ResponseEntity<ApiResponse<?>> handle(Exception ex, ResponseCode code, ResponseType type) {
		return ResponseEntity.ok(ApiResponse.error(code.getValue(),
				code.compareTo(ResponseCode.INTERNAL_SERVER_ERROR) == 0 ? rb.getString("error.server") : ex.getMessage(),
				type.getValue()));
	}
}
